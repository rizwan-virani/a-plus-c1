/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `APLUS1` namespace with
   ...
   - quizEngine.js adds the 500+ question bank, the 150-object PBQ database, and
...
(function () {
  ...
  var S = window.APLUS1 || {};
  var PASS = (S.exam && S.exam.passing) || 750;
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("aplus1." + k, fb); },
      set: function (k, v) { write("aplus1." + k, v); },
      del: function (k) { try { localStorage.removeItem("aplus1." + k); } catch (e) {} }
    };
  })();
  ...
  function statCard() {
    ...
    return el("div.kpis", null, [
      ...
      kpi(pbqDone + "/" + (S.pbqs ? S.pbqs.length : 150), "PBQs completed", ""),
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the 220-1201 is scored and delivered, and where the credential fits in an IT/cybersecurity career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the 220-1201 &mdash; the high-stakes details every candidate should internalize before sitting the test.",
      foot: "90 min · ≤90 items · " + PASS + " to pass", cta: "Read",
      onClick: function () { openExamMechanics(); }
    }));
    g1.appendChild(card({
      icon: "🧭", tag: "CAREER", title: "Career Guidance", tagCls: "ghost",
      desc: "Where A+ sits as the entry point to an IT career: the first credential many employers ask for, and the help-desk, desktop-support, and field-technician roles it unlocks.",
      foot: "The entry-level IT credential", cta: "Read",
    ...
    b2.appendChild(el("p.sectsub", { text: "Five domains, each a rigorous reading interface with deep technical definitions, comparison tables, exam tips, and real-world scenarios." }));
    var g2 = el("div.grid");
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every A+ acronym across all five domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    /* ---- 04 · Practice Quizzes ---- */
    var b4 = dashSection("05", "Practice quizzes", (S.questions ? S.questions.length : "500+") + " questions · 4 modes");
    b4.appendChild(el("p.sectsub", { html: "Drawn at random from a bank of <b>" + (S.questions ? S.questions.length : "500+") + "</b> original items, graded with full rationale. Domain quizzes isolate one objective set; the quick quiz mixes all five." }));
    ...
    g3b.appendChild(card({
      ...
      desc: "Ten questions drawn uniformly from all five domains &mdash; a fast, mixed knowledge check with full rationale on every item.",
    ...
    var b6 = dashSection("06", "Performance-based question simulators", (S.pbqs ? S.pbqs.length : 150) + " simulations · 6 modules");
    b6.appendChild(el("p.sectsub", { html: "A database of <b>" + (S.pbqs ? S.pbqs.length : 150) + "</b> hands-on PBQs across five formats. Read the exhibit, configure each field, and submit for graded feedback mapped to the objective." }));
    var g4 = el("div.grid");
    ...
    /* ---- 07 . Hands-on labs ---- */
    var bLabs = dashSection("07", "Hands-on labs", "20 interactive sandbox labs");
    ...
    var b5 = dashSection("08", "Mock certification exam", "90 questions · timed");
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the 220-1201 &mdash; 90 questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    g3c.appendChild(card({
      icon: "🧪", tag: "MOCK EXAM", title: "Full-Length Mock Exam",
      desc: "A realistic 90-minute simulation: 90 questions weighted to the exact domain percentages, with a countdown timer, question flagging, and a domain-by-domain scoring dashboard.",
      foot: "90 questions · 90 minutes · timed", cta: "Begin",
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: Professor Messer's full 220-1201 course, the official CompTIA objectives PDF, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.APLUS1_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function computeReadiness() {
    ...
    var pbqTotal = (S.pbqs || []).length || 150, pbqDone = (Store.get("pbqDone", []) || []).length;
    ...
    var dims = [
      ...
      { key: "mock", label: "Mock exam", weight: 1.6, value: bestMock != null ? Math.round(bestMock / 900 * 100) : null, has: mocks.length > 0, kind: "score", detail: bestMock != null ? ("best " + bestMock + "/900") : "not attempted" },
  ...
  function readinessOutcome(R) {
    if (R.bestMock != null) {
      var m = R.bestMock;
      if (m >= PASS + 50) return { cls: "good", text: "Strong pass projected. Your best mock of " + m + " out of 900 clears the " + PASS + " passing line with margin." };
      if (m >= PASS) return { cls: "good", text: "On track to pass. Your best mock of " + m + " out of 900 is at or above the " + PASS + " passing line. Keep it consistent." };
      if (m >= PASS - 70) return { cls: "warn", text: "Borderline. Your best mock of " + m + " out of 900 sits just under the " + PASS + " line. A focused push should get you there." };
      return { cls: "crit", text: "Not yet. Your best mock of " + m + " out of 900 is below passing. Keep building before you book the exam." };
    }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates APLUS1.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.APLUS1_VER ? "?v=" + window.APLUS1_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "220-1201 Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: 220-1201", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where A+ fits in your path", S.careerGuidance || [])); }
      ...
      tag: "Analytics", render: function (host) {
        ...
        h.slice(0, 60).forEach(function (r) {
          ...
          var scoreCell = r.mode === "mock"
            ? el("span.pillscore." + (pass ? "pass" : "fail"), { text: r.scaled + " / 900" })

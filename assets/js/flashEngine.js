/* ============================================================================
   a+ core 1  ::  flashEngine.js  —  Rapid Recall Flashcards
   ...
   - Cards live in lazy-loaded per-domain modules (assets/js/content/flashN.js),
     each populating APLUS1.flash[N] with 100 cards { id, d, t, obj, front, back }.
   - Scheduling state persists to localStorage (aplus1.flash.v1) keyed by card id,
...
(function () {
  ...
  function ver() { return window.APLUS1_VER ? "?v=" + window.APLUS1_VER : ""; }
  ...
  function loadAll(cb) {
    var need = [1, 2, 3, 4, 5].filter(function (n) { return !(S.flash && S.flash[n] && S.flash[n].length); });
  ...
  function allCards() { var out = []; [1, 2, 3, 4, 5].forEach(function (n) { out = out.concat(S.flash[n] || []); }); return out; }
  ...
  window.FLASH = {
    ...
    masterAcronymCount: function () { return (S.flash && [1, 2, 3, 4, 5].every(function (n) { return S.flash[n]; })) ? masterAcronyms().length : null; },
    ...
    analytics: function () {
      var st = store();
      var agg = { total: 500, studied: 0, mastered: 0, learning: 0, reps: 0, lapses: 0 };
      var byD = {}; for (var n = 1; n <= 5; n++) byD[n] = { total: DECK_SIZE, studied: 0, mastered: 0, due: this.domainDue(n) };
      for (var id in st) {
